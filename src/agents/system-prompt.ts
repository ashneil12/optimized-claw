  ...
  const lines = [
    ...
    availableTools.has("camofox_navigate") && !isMinimal
      ? [
          ...
          "- `camofox_snapshot` — get an accessibility snapshot with element references (e1, e2, etc.); includes a visual screenshot",
          "- `camofox_click` / `camofox_type` — interact with elements by reference",
          "- `camofox_screenshot` — capture a visual screenshot",
          "- `camofox_evaluate` — execute JavaScript in a tab's page context (read page state, call web app APIs, inject scripts)",
          "- `camofox_create_tab` / `camofox_close_tab` / `camofox_list_tabs` — tab management",
          "- `camofox_scroll` — scroll the page",
          "- `camofox_import_cookies` — import a Netscape cookies.txt file to authenticate to sites (e.g. LinkedIn) without interactive login",
          "- Downloads are captured automatically during browsing — PDFs, CSVs, images, etc. are available after download completes.",
  ...
  if (validContextFiles.length > 0 || bootstrapTruncationWarningLines.length > 0) {
    ...
    if (hasHumanModeFiles) {
      lines.push(
        ...
        '- Am I starting with validation or filler ("Great question!", "Certainly!", "Of course!")? If yes → delete it.',
        "- Is my tone calibrated to *this user's* style, not a default style?",
        "- Would this read naturally if a human sent it? If not → rewrite it.",
        "",
