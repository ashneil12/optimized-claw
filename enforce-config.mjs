...
function ensureAgentBrowserContainers(configPath) {
  // DISABLED: browser containers are managed by docker-compose.override.yml
  // (generated by ensure-agent-browsers.sh on the host).
  //
  // Using raw `docker create` here created standalone containers (e.g. `browser-ocs-nehemiah`)
  // that coexisted with the compose-managed ones (`moltbot-browser-ocs-nehemiah-1`).
  // Both registered on the Docker network with the same service alias, causing
  // dual DNS entries and intermittent CDP 404 errors when Playwright connected
  // to the wrong Chrome instance.
  console.log(
